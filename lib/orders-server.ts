...
export type CustomerOrder = {
  ...
  created_at: string;
  items: Array<{ id: string; product_name_snapshot: string; sku_snapshot: string; quantity: number; unit_price_idr: number; item_type: string }>;
...
export async function getCustomerOrders(): Promise<CustomerOrder[]> {
  ...
  const [{ data: items }, { data: shipments }] = await Promise.all([
    (supabase.from('commerce_order_items') as any).select('id, order_id, product_name_snapshot, sku_snapshot, quantity, unit_price_idr, item_type').in('order_id', orderIds),
