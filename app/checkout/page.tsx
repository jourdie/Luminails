...
export default async function CheckoutPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  if (!item) return <><SiteNavigation identity={account.identity} profile={account.profile} tierSummary={account.tierSummary} /><main className={'checkout-page'}><div className={'checkout-shell'}><h1>Pilih package<br /><em>untuk mulai.</em></h1><Link className={'brand-button brand-button-dark'} href={'/packages'}>Browse packages <span>-&gt;</span></Link></div></main></>;
