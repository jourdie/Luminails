...
export function Storefront({ products }: { products: CatalogProduct[] }) {
  ...
  return (
    <>
      ...
      <header className="site-header">
        <a className="brand" href="#top" aria-label="Luminails home">luminails<span className="brand-dot">.</span></a>
        <nav className={`main-nav${mobileMenu ? ' mobile-open' : ''}`} aria-label="Navigasi utama">
          <a href="#catalog" onClick={() => setMobileMenu(false)}>Belanja</a>
          <a href="#collections" onClick={() => setMobileMenu(false)}>Koleksi</a>
          <a href="#how-it-works" onClick={() => setMobileMenu(false)}>Cara kerja</a>
        </nav>
        <div className="header-actions">
          <button className="text-button" onClick={() => setLoginOpen(true)}>Masuk B2B</button>
          <button className="cart-button" onClick={() => setCartOpen(true)} aria-label={`Buka keranjang, ${cartCount} item`}><span>Keranjang</span><b className="cart-count">{cartCount}</b></button>
        </div>
      ...
      <main id="top">
        <section className="hero section-pad">
          <div className="hero-copy">
            ...
            <div className="hero-actions"><a className="button button-dark" href="#catalog">Lihat katalog <span>↗</span></a><button className="button button-quiet" onClick={() => setLoginOpen(true)}>Daftar akun B2B</button></div>
        ...
        <section className="catalog-section section-pad" id="catalog">
          <div className="section-heading"><div><p className="eyebrow">Katalog terkurasi</p><h2>Masuk keranjang<br /><em>minggu ini.</em></h2></div><p className="section-intro">Pilih essentials yang paling sering dicari oleh salon, nail artist, dan studio kecantikan.</p></div>
      ...
      <footer className="site-footer section-pad"><a className="brand" href="#top">luminails<span className="brand-dot">.</span></a><p>Nail supply untuk tangan yang selalu bekerja.</p><div className="footer-links"><a href="#catalog">Katalog</a><a href="#how-it-works">Cara kerja</a><a href="#top">Bantuan</a></div><small>© 2026 Luminails Commerce · Next.js + Supabase foundation</small></footer>
