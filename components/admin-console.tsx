...
export function AdminConsole({ dashboard }: { dashboard: AdminDashboard }) {
  ...
  if (dashboard.access === 'denied') {
    return <main className="admin-shell"><AdminHeader /><section className="admin-denied"><span className="admin-symbol">X</span><h1>Akses admin belum diberikan.</h1><p>Akun Anda sudah login, tetapi belum ada baris aktif di <code>admin_memberships</code>.</p><Link className="button button-dark" href="/auth?next=/admin">Masuk sebagai staff</Link></section></main>;
  }
...
function OrderRow({ order, expanded }: { order: AdminDashboard['orders'][number]; expanded?: boolean }) {
  return <div className={`order-row${expanded ? ' order-row-expanded' : ''}`}><div><b>{order.id.slice(0, 14)}</b><small>{date(order.created_at)}</small></div><span className={`status-pill status-${order.status}`}>{order.status.replaceAll('_', ' ')}</span><span className="payment-copy">{order.payment_status === 'paid' ? 'Sudah dibayar' : 'Belum dibayar'}</span><strong>{money(order.total_idr)}</strong>{expanded && <button className="underlined-link">Buka detail <span>&rarr;</span></button>}</div>;
}
