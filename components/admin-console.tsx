...
export function AdminConsole({ dashboard }: { dashboard: AdminDashboard }) {
  const [tab, setTab] = useState<'overview' | 'products' | 'orders' | 'pricing'>('overview');
  const unread = dashboard.notifications.filter((notification) => !notification.read_at).length;

  if (dashboard.access === 'denied') {
    return <main className="admin-shell"><AdminHeader /><section className="admin-denied"><span className="admin-symbol">×</span><h1>Akses admin belum diberikan.</h1><p>Akun Anda sudah login, tetapi belum ada baris aktif di <code>admin_memberships</code>.</p><Link className="button button-dark" href="/auth?next=/admin">Masuk sebagai staff</Link></section></main>;
  }

  const canEditPricing = dashboard.access === 'granted' && dashboard.role === 'owner';
  return <main className="admin-shell"><AdminHeader demo={!dashboard.configured} role={dashboard.role} /><div className="admin-layout"><aside className="admin-sidebar"><p className="eyebrow">Workspace</p>{[['overview', 'Ringkasan'], ['products', 'Produk'], ['orders', 'Transaksi'], ['pricing', 'Pricing B2B']].map(([value, label]) => <button key={value} className={`admin-nav-item${tab === value ? ' is-active' : ''}`} onClick={() => setTab(value as typeof tab)}>{label}{value === 'orders' && unread > 0 && <b>{unread}</b>}</button>)}<div className="admin-sidebar-foot"><Link href="/">← Storefront</Link><span>Role: {dashboard.role ?? 'demo owner'}</span></div></aside><section className="admin-content">{dashboard.access === 'demo' && <div className="demo-banner">Mode preview aktif — data di bawah adalah demo. Isi Supabase + login admin untuk menyimpan perubahan nyata.</div>}{tab === 'overview' && <Overview dashboard={dashboard} unread={unread} onNotifications={() => setTab('orders')} />}{tab === 'products' && <Products products={dashboard.products} />}{tab === 'orders' && <Orders orders={dashboard.orders} notifications={dashboard.notifications} />}{tab === 'pricing' && <Pricing tiers={dashboard.pricingTiers} canEdit={canEditPricing} />}</section></div></main>;
}
...
function OrderRow({ order, expanded }: { order: AdminDashboard['orders'][number]; expanded?: boolean }) {
  return <div className={`order-row${expanded ? ' order-row-expanded' : ''}`}><div><b>{order.id.slice(0, 14)}</b><small>{date(order.created_at)}</small></div><span className={`status-pill status-${order.status}`}>{order.status.replaceAll('_', ' ')}</span><span className="payment-copy">{order.payment_status === 'paid' ? 'Sudah dibayar' : 'Belum dibayar'}</span><strong>{money(order.total_idr)}</strong>{expanded && <button className="underlined-link">Buka detail <span>↗</span></button>}</div>;
}
